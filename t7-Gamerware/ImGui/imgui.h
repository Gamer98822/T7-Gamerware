...
// 3D vector (often used to store coordinates)
struct ImVec3
{
    float                                   x, y, z;
    ImVec3()                                { x = y = z = 0.0f; }
    ...
    IM_VEC3_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.h to convert back and forth between your math types and ImVec3.
#endif
};
...
namespace ImGui
{
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol idx, ImGuiCol col);
    ...
    IMGUI_API const char*   GetKeyName(int k);
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CharsUnsigned       = 1 << 0,   // Allow 0123456789
    ImGuiInputTextFlags_CharsSigned         = 1 << 1,   // Allow 0123456789-
    ImGuiInputTextFlags_CharsIPAddress      = 1 << 2,   // Allow 0123456789.
    ImGuiInputTextFlags_CharsDecimal        = 1 << 3,   // Allow 0123456789.+-*/
    ImGuiInputTextFlags_CharsHexadecimal    = 1 << 4,   // Allow 0123456789ABCDEFabcdef
    ImGuiInputTextFlags_CharsUppercase      = 1 << 5,   // Turn a..z into A..Z
    ImGuiInputTextFlags_CharsNoBlank        = 1 << 6,   // Filter out spaces, tabs
    ImGuiInputTextFlags_AutoSelectAll       = 1 << 7,   // Select entire text when first taking mouse focus
    ImGuiInputTextFlags_EnterReturnsTrue    = 1 << 8,   // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking at the IsItemDeactivatedAfterEdit() function.
    ImGuiInputTextFlags_CallbackCompletion  = 1 << 9,   // Callback on pressing TAB (for completion handling)
    ImGuiInputTextFlags_CallbackHistory     = 1 << 10,   // Callback on pressing Up/Down arrows (for history handling)
    ImGuiInputTextFlags_CallbackAlways      = 1 << 11,   // Callback on each iteration. User code may query cursor position, modify text buffer.
    ImGuiInputTextFlags_CallbackCharFilter  = 1 << 12,   // Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or return 1 in callback to discard.
    ImGuiInputTextFlags_AllowTabInput       = 1 << 13,  // Pressing TAB input a '\t' character into the text field
    ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 14,  // In multi-line mode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with Ctrl+Enter, add line with Enter).
    ImGuiInputTextFlags_NoHorizontalScroll  = 1 << 15,  // Disable following the cursor horizontally
    ImGuiInputTextFlags_AlwaysInsertMode    = 1 << 16,  // Insert mode
    ImGuiInputTextFlags_ReadOnly            = 1 << 17,  // Read-only mode
    ImGuiInputTextFlags_Password            = 1 << 18,  // Password mode, display all characters as '*'
    ImGuiInputTextFlags_NoUndoRedo          = 1 << 19,  // Disable undo/redo. Note that input text owns the text data while active, if you want to provide your own undo/redo stack you need e.g. to call ClearActiveID().
    ImGuiInputTextFlags_CharsScientific     = 1 << 20,  // Allow 0123456789.+-*/eE (Scientific notation input)
    ...
    // [Internal]
    ImGuiInputTextFlags_Multiline           = 1 << 22,  // For internal use by InputTextMultiline()
    ImGuiInputTextFlags_NoMarkEdited        = 1 << 23   // For internal use by functions using InputText() before reformatting data
};
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontDefault(const float size_pixels = 13.0f, const ImFontConfig* font_cfg = NULL);
