...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    RenderNavHighlight(total_bb, id);
    float pad = ImMax(1.0f, IM_FLOOR(square_sz / 5.0f));
    window->DrawList->AddCircleFilled(center, radius - pad, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        pad = ImMax(1.0f, IM_FLOOR(square_sz / 4.25f));
        window->DrawList->AddCircleFilled(center, radius - pad, GetColorU32(ImGuiCol_CheckMark), 16);
    }

    if (style.FrameBorderSize > 0.0f)
    {
        pad = ImMax(1.0f, IM_FLOOR(square_sz / 5.0f));
        window->DrawList->AddCircle(center + ImVec2(1,1), radius - pad, GetColorU32(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
        window->DrawList->AddCircle(center, radius - pad, GetColorU32(ImGuiCol_Border), 16, style.FrameBorderSize);
    }
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    if (flags & (ImGuiInputTextFlags_CharsUnsigned | ImGuiInputTextFlags_CharsSigned | ImGuiInputTextFlags_CharsIPAddress | ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific))
    {
        if (flags & ImGuiInputTextFlags_CharsUnsigned)
            if (!(c >= '0' && c <= '9'))
                return false;

        if (flags & ImGuiInputTextFlags_CharsSigned)
            if (!(c >= '0' && c <= '9') && (c != '-'))
                return false;

        if (flags & ImGuiInputTextFlags_CharsIPAddress)
            if (!(c >= '0' && c <= '9') && (c != '.'))
                return false;
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    float height_in_items_f = (height_in_items + 0.00f);
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    ImGuiSelectableFlags flags = ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_SetNavIdOnHover | (enabled ? 0 : ImGuiSelectableFlags_Disabled);
    bool pressed;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
    ...
    else
    {
        ...
        if (selected)
            RenderCheckMark(window->DrawList, pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetColorU32(ImGuiCol_CheckMark), g.FontSize  * 0.866f);
    }
