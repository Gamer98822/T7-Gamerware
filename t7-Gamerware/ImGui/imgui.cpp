...
void ImGui::PushStyleColor(ImGuiCol idx, ImGuiCol col)
{
    ImGuiContext& g = *GImGui;
    ImGuiColorMod backup;
    backup.Col = idx;
    backup.BackupValue = g.Style.Colors[idx];
    g.ColorModifiers.push_back(backup);
    g.Style.Colors[idx] = g.Style.Colors[col];
}
...
const char* ImGui::GetKeyName(int k)
{
    UINT scanCode = MapVirtualKey(k, MAPVK_VK_TO_VSC);
    switch (k)
    {
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    if (g.ActiveId == id) {
        for (auto i = VK_BACK; i <= VK_OEM_CLEAR; i++) {
            if (io.KeysDown[i]) {
                key = i;
                value_changed = true;
                ClearActiveID();
    ...
    if (*k != 0 && g.ActiveId != id) {
        strcpy_s(buf_display, GetKeyName(*k));
    }
    else if (g.ActiveId == id) {
        strcpy_s(buf_display, "<Press a key>");
    }

    const ImRect clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + frame_size.x, frame_bb.Min.y + frame_size.y); // Not using frame_bb.Max because we have adjusted size
