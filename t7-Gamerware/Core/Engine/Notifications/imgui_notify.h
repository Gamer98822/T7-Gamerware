...
// Comment out if you don't want any duration indicator at the bottom
#define NOTIFY_USE_DURATION_BAR
...
public:
	...
	NOTIFY_INLINE auto get_creation_time() { return this->creation_time; }

	NOTIFY_INLINE auto get_dismiss_time() { return NOTIFY_FADE_IN_OUT_TIME + this->dismiss_time + NOTIFY_FADE_IN_OUT_TIME; }

	NOTIFY_INLINE auto get_elapsed_time() { return GetTickCount64() - this->creation_time; }

	NOTIFY_INLINE auto get_remaining_time() { return this->creation_time + this->dismiss_time - GetTickCount64(); }
...
namespace ImGui
{
	...
	{
		const auto vp_size = GetIO().DisplaySize;
		const auto border_size = GetStyle().WindowBorderSize;
...
#ifdef NOTIFY_USE_DURATION_BAR
				const auto fraction = (float)current_toast->get_elapsed_time() / (float)current_toast->get_dismiss_time();

				const auto window_pos = GetWindowPos();
				const auto window_size = GetWindowSize();

				const ImVec2 bar_pos(window_pos.x + border_size, window_pos.y + border_size);
				const ImVec2 bar_size(window_size.x - border_size * 2.0f, window_size.y - border_size * 2.0f);

				const ImVec2 p_min(bar_pos.x, bar_pos.y + bar_size.y - 2.0f);
				const ImVec2 p_max(bar_pos.x + bar_size.x * fraction, bar_pos.y + bar_size.y);

				GetWindowDrawList()->AddRectFilled(p_min, p_max, GetColorU32(ImGuiCol_Header));
#endif
			}
